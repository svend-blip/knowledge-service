...
| GET | `/v1/learning/drafts` | `pending` | List learning drafts under the runs root with their admission state |
| GET | `/v1/retrievals` | `run_id`, `handoff_id`, `flow_key`, `agent_role`, `scope`, `since`, `until`, `limit`, `offset`, `summary` | Read the retrieval log, or its aggregate with `summary=true` |
...
- Python >= 3.11 (3.12 is the documented target for the portable provider).
- Provider `leann` (Linux): a CUDA GPU with about 2.5 GB of free VRAM at
  query time, and the pinned `leann` and `torch` packages in
  `requirements.txt`. See "GPU requirement" and "Coexistence budget".
- Provider `portable` (any platform): CPU only, `requirements-portable.txt`.
- No external service at run time: SQLite, local index files and a local
  embedding model.

### Ecosystem dependencies and installation order

**This repository is row 3.** It is a leaf at run time. LEANN is used here and nowhere else in the ecosystem; agents reach it through mcp-light, and DPMtF-WebUI reaches it directly over HTTP.
